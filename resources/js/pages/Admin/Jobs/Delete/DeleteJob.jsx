...
const DeleteJob = () => {
  ...
  const { job, appURL, customErrors, passwordTries } = usePage().props
  ...
  const submitSoftDelete = () => {
    setTries(t => t + 1)
    Inertia.post(route('admin.jobs.delete', job.id), {
      deleteType: 'soft',
      password: password,
  ...
  return (
    <AdminLayout>
      ...
      <Head title={ translate('delete-job') } />

      <AdminHeader title={ translate('delete-job') } icon={faTrash} />
