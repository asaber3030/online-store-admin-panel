...
const DeleteUser = () => {
  ...
  const { user, appURL, customErrors, passwordTries } = usePage().props
  ...
  const submitSoftDelete = () => {
    setTries(t => t + 1)
    Inertia.post(route('admin.users.delete', user.id), {
      deleteType: 'soft',
      password: password,
  ...
  return (
    <AdminLayout>
      ...
      <Head title={ translate('delete-user') } />

      <AdminHeader title={ translate('delete-user') } icon={faTrash} />
      ...
      <div className="delete-structure">
          ...
          <div className="delete-alert">
            <div className="soft-delete">
              ...
              <div className="soft-delete-actions">
                ...
                <Link className='btn btn-primary btn-sm p-2' href={route('admin.users.list')}><FontAwesomeIcon icon={faTimes} /> {translate('cancel')}</Link>
              </div>
            </div>
        ...
        <Modal centered backdrop={'static'} keyboard={false} size={'xl'} show={passwordModal} onHide={ () => setPasswordModal(false) }>
          ...
          <Modal.Footer>
            ...
            <ActionLink title={translate('cancel')} icon={faTimes} href={route('admin.users.list')} color='secondary' />
          </Modal.Footer>
        </Modal>
